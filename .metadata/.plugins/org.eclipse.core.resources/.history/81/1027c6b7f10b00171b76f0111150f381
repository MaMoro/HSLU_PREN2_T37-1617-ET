...
uint8_t VL_ChipEnable(uint8_t deviceNo, bool on) {
  ...
  if (deviceNo==0) {
    if (on) {
      TofCE1_SetInput(TofCE1_DeviceData); /* let it pull high by the pull-up on the board */
    } else { /* pull it down */
      ...
      if (on) {
        TofCE2_SetInput(TofCE2_DeviceData); /* let it pull high by the pull-up on the board */
      } else { /* pull it down */
...
uint8_t VL_Init(void) {
	  ...
	  for(i=0;i<VL_MULTIPLE_DEVICES;i++) {
	    ...
	    if (res!=ERR_OK) {
	     // CLS1_SendStr("ERROR: Failed set i2C address of TOF device: ", CLS1_GetStdio()->stdErr);
