...
uint8_t MMA1_GetRaw8XYZ(uint8_t *xyz)
{
  ...
  return GI2C2_ReadAddress(MMA1_I2C_ADDR, (uint8_t*)&addr, sizeof(addr), &xyz[0], 3);
}
...
uint8_t MMA1_Init(void)
{
  ...
  return GI2C2_WriteByteAddress8(MMA1_I2C_ADDR, MMA1_CTRL_REG_1, MMA1_ACTIVE_BIT_MASK); /* enable device */
}
...
uint16_t MMA1_MeasureGetRawX(void)
{
  ...
  if(GI2C2_ReadAddress(MMA1_I2C_ADDR, (uint8_t*)&addr, sizeof(addr), &val.buf[0], sizeof(val.buf))!=ERR_OK) {
    return 0; /* failure */
  }
...
uint8_t MMA1_SetFastMode(bool on)
{
  ...
  res = GI2C2_ReadByteAddress8(MMA1_I2C_ADDR, MMA1_CTRL_REG_1, &val);
  if (res!=ERR_OK) {
    return res;
  }
  if (on) {
  ...
  return GI2C2_WriteByteAddress8(MMA1_I2C_ADDR, MMA1_CTRL_REG_1, val);
}
...
{
  return GI2C2_ReadByteAddress8(MMA1_I2C_ADDR, MMA1_WHO_AM_I, value);
}
...
{
  if (GI2C2_ReadByteAddress8(MMA1_I2C_ADDR, addr, val)!=ERR_OK) {
    return ERR_FAILED;
  }
  return ERR_OK;
}
...
{
  return GI2C2_WriteByteAddress8(MMA1_I2C_ADDR, addr, val);
}
...
uint8_t MMA1_SwReset(void)
{
  ...
  res = GI2C2_ReadByteAddress8(MMA1_I2C_ADDR, MMA1_CTRL_REG_2, &val);
  if (res!=ERR_OK) {
    return res;
  }
  val |= MMA1_CTRL_REG_2_MASK_RST; /* reset device */
  res = GI2C2_WriteByteAddress8(MMA1_I2C_ADDR, MMA1_CTRL_REG_2, val);
  if (res!=ERR_OK) {
    return res;
  }
